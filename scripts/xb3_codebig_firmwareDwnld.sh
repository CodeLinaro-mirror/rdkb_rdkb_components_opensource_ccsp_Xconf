...
wan_mac_interface=$(getWanMacInterfaceName)
...
getFirmwareUpgDetail()
{
    ...
    xconf_retry_count=0
    retry_flag=1
    isIPv6=`ifconfig $WAN_INTERFACE | grep inet6 | grep -i 'Global'`
	...
	if [ "$devicemodel" == "" ];then
        ...
        MAC=`ifconfig  | grep $wan_mac_interface |  grep -v $wan_mac_interface:0 | tr -s ' ' | cut -d ' ' -f5`
                date=`date`
...
getMacAddress()
{
        ifconfig  | grep $wan_mac_interface |  grep -v $wan_mac_interface:0 | tr -s ' ' | cut -d ' ' -f5
}
